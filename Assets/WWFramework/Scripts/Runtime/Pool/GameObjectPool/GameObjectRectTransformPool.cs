/*------------------------------
 * 脚本名称: GameObjectRectTransformPool
 * 创建者: movin
 * 创建日期: 2025/03/29
------------------------------*/
...
namespace WWFramework
{
    ...
    /// </summary>
    public class GameObjectRectTransformPool : MonoBehaviour, IGameObjectPool
    {
        /// <summary>
        /// 闲置引用列表
        /// </summary>
            ...
            Func<GameObject, UniTask> onDespawnToPool = null)
        {
            ...
            RectTransform rectTrans = this.GetOrAddComponent<RectTransform>();
        ...
        /// <returns></returns>
        public async UniTask<GameObject> Spawn(RectTransform parent = null)
        {
            // 校验引用池模板
            if (null == _gameObjectPrefab)
            {
                Log.LogError(sb =>
                {
                    sb.Append("GameObjectTransformPool: Spawn gameObjectPrefab is null");
                });
                return null;
            }
